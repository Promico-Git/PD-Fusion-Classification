...
def preprocess_audio(audio_path, augment=False, for_rnn=False):
    ...
    if for_rnn:
        ...
        # Stack them to get a richer feature set (e.g., 40 + 40 + 40 = 120 features per frame)
        # Resulting shape: (num_features, num_frames)
        features = np.concatenate((mfccs, delta_mfccs, delta2_mfccs))

        # Transpose to (num_frames, num_features) for RNN input (sequence_length, input_size)
        features = features.T

        # Padding/Truncation for fixed sequence length
        max_seq_len = 200 # You can adjust this value based on your audio lengths
        ...
        # Normalize features
        # A simple min-max scaling per feature dimension
...
class ParkinsonDataset(Dataset):
    ...
    def __getitem__(self, idx):
        ...
        if not self.for_rnn:
            audio_features = torch.tensor(audio_features).unsqueeze(0) # Add channel dimension for CNN
...
class AudioRNN(nn.Module):
    def __init__(self, input_size, hidden_size, num_layers, num_classes, bidirectional=True, dropout_rate=0.3):
        ...
        # Output of LSTM (h_n) is (num_directions * num_layers, batch_size, hidden_size)
        # We'll take the output of the last layer, potentially from both directions
    ...
    def forward(self, x):
        ...
        # We take the output from the last layer (h_n) for classification.
...
if __name__ == "__main__":
    ...
    # Determine input_size for RNN:
    # Load one sample to get the feature dimensions
    temp_features = preprocess_audio(train_audio[0], for_rnn=True)
    rnn_input_size = temp_features.shape[1] # Should be 120 (40 MFCCs + 40 Deltas + 40 Delta-Deltas)
    rnn_max_seq_len = temp_features.shape[0] # Should be 200

    # RNN Model Parameters
    rnn_hidden_size = 128
    rnn_num_layers = 2
    rnn_num_classes = 2 # Assuming binary classification (healthy/patient)
